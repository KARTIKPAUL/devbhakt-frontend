...
function getToken() {
  ...
  try {
    return localStorage.getItem("kartikeyo_token");
  } catch {
    return null;
  }
}
...
async function request(path, { method = "GET", body, auth = false, headers = {} } = {}) {
  ...
  try {
    ...
    const error = new Error(
      "Could not reach the Kartikeyo server. Please check your connection and try again."
    );
...
export const api = {
  ...
  // Admin — Products
  getAllProductsAdmin: () => request("/api/products/admin/all", { auth: true }),
  createProduct: (formData) => request("/api/products", { method: "POST", body: formData, auth: true }),
  updateProduct: (id, formData) =>
    request(`/api/products/${id}`, { method: "PUT", body: formData, auth: true }),
  ...
  getAllOrdersAdmin: (params = {}) => {
    ...
    return request(`/api/orders${query ? `?${query}` : ""}`, { auth: true });
  },
  updateOrderStatus: (id, orderStatus) =>
    request(`/api/orders/${id}/status`, { method: "PUT", body: { orderStatus }, auth: true }),
};
