...
Kartikeyo devotional clothing & lifestyle backend. Built with Tailwind CSS using the brand's
saffron / black / cream color theme from the Kartikeyo logo.
...
Open http://localhost:3000. Make sure the Kartikeyo backend is running (default
...
| Variable | Purpose |
|---|---|
| `NEXT_PUBLIC_API_URL` | Base URL of the Kartikeyo backend API (no trailing slash) |
...
- Cart state lives in `localStorage` (`Kartikeyo_cart`) and prices are always re-verified by the
  backend when the order is created — nothing here is trusted for final pricing.
- Auth token is stored in `localStorage` (`Kartikeyo_token`) and attached as a Bearer token to
  protected API calls.
